...
# 1 without rest and NO model query func based view (FBV)
def no_rest_no_model(request):
    guests = [
        {"id": 1, "name": "ahmed", "mobile": 12131415},
        {"id": 2, "name": "mohamed", "mobile": 21314151},
    ]
    return JsonResponse(guests, safe=False)


# 2 without rest and from model default
def no_rest_from_model(request):
    data = Guest.objects.all()
    response = {"guests": list(data.values("name", "mobile_num"))}
...
def new_reservation(request):
    ...
    guest.mobile_num = request.data["mobile_num"]
    guest.save()
